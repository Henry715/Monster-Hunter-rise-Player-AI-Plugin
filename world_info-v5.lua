...
local socket=dofile("F:\\SteamLibrary\\steamapps\\common\\MonsterHunterRise\\socket\\socket.lua")
...
function this.init_server()
    server = assert(socket.bind("*", 8080))
    server:settimeout(0) -- NON-BLOCKING
    print("AI server started on port 8080")
end

function this.ai_network_update()
    ...
    if client then
        ...
        local ok, err = client:send(json_str .. "\n")
...
function this.safe_send(msg)
    if not client then return end

    -- ensure newline framing (VERY important for Python reading)
    msg = msg .. "\n"
...
function this.extract_highlighted_monster()
	...
	-- 	return nil;
	-- end
	if large_monster.highlighted_id == -1 or singletons.enemy_manager == nil then
		return {
			name = "",
			id = -1,
			unique_id = -1,
			is_large = false,
			dead_or_captured = false,
			position = { x = 0, y = 0, z = 0 },
			head_position = { x = 0, y = 0, z = 0 },
			health = { current = 0, max = 0, percentage = 0, missing = 0 },
			capture = { health = 0, percentage = 0, is_capturable = false },
			stamina = { current = 0, max = 0, percentage = 0, is_tired = false, tired_timer = 0, tired_duration = 0 },
			rage = { point = 0, limit = 0, percentage = 0, is_in_rage = false, timer = 0, duration = 0 },
			stealth = { is_stealth = false, can_go_stealth = false },
			size = { value = 0, small_border = 0, big_border = 0, king_border = 0, crown = "" },
			rider_id = -1,
			is_anomaly = false,
			parts = {},
			ailments = {}
